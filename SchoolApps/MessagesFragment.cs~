...
namespace SchoolApps
{
	public class MessagesFragment : Fragment
	{
		...
		//List<TableItem> tableItemizeds = new List<TableItem> ();
		...
		public override View OnCreateView(LayoutInflater layoutInflater, ViewGroup viewGroup, Bundle savedInstanceState)
		{
			...
			var rootView = layoutInflater.Inflate (Resource.Layout.MessageLayout, viewGroup, false);

			itemsListView = (ListView) rootView.FindViewById (Resource.Id.lvMessagesStandalone);
		...
		public void clickedMessage(object sender, Android.Widget.AdapterView.ItemClickEventArgs e) {
			...
			Toast.MakeText (Activity, "Clicked on position " + e.Position, ToastLength.Short).Show();
